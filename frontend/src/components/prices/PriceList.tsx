    ...
    <div className={className}>
      <Card className="shadow-sm">
        <CardHeader className="pb-4 px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col space-y-4 sm:space-y-0 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex-1 min-w-0">
              {/* Mobile: Hide title and description as they're now in PricesPage */}
              ...
              {/* Mobile: Show compact header */}
              <div className="block md:hidden">
                <CardTitle className="text-lg font-bold text-gray-900 dark:text-white">
                  {t('prices.results', 'Résultats')}
                </CardTitle>
            ...
            <div className="flex items-center gap-2 sm:hidden">
              <Button
                ...
                className="prices-button px-3 py-2 h-10"
              >
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
              <Link to="/map" className="w-full sm:w-auto">
                <Button 
                  variant="outline" 
                  className="prices-button w-full sm:w-auto justify-center h-11 px-4 sm:px-6 border-2 border-blue-200 dark:border-blue-800 hover:border-blue-300 dark:hover:border-blue-700 hover:bg-blue-50 dark:hover:bg-blue-900/20 text-blue-700 dark:text-blue-300 transition-all duration-200"
                >
                  <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                  </svg>
                  <span className="hidden sm:inline">{t('prices.mapView', 'Vue Carte')}</span>
                  <span className="sm:hidden">{t('prices.mapView', 'Carte')}</span>
                </Button>
              </Link>
              <Link to="/submit" className="w-full sm:w-auto">
                <Button className="prices-button w-full sm:w-auto justify-center h-11 px-4 sm:px-6 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 dark:from-green-600 dark:to-emerald-600 dark:hover:from-green-700 dark:hover:to-emerald-700 text-white font-medium transition-all duration-200 shadow-lg hover:shadow-xl">
                  <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                  </svg>
                  <span className="hidden sm:inline">{t('prices.submit', 'Soumettre Prix')}</span>
                  <span className="sm:hidden">{t('prices.submit', 'Soumettre')}</span>
                </Button>
              </Link>
            </div>
          </div>
        ...
        <CardContent className="space-y-6 px-4 sm:px-6 lg:px-8">
          ...
          {/* Quick Overview for Mobile - Enhanced */}
          <div className="sm:hidden">
            <div className="prices-overview-card bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-700 rounded-2xl p-4 mb-4 shadow-sm border border-blue-100 dark:border-gray-600">
              <PriceOverview prices={prices} className="mb-0" />
            </div>
          </div>

          {/* Results Info - Enhanced Mobile Design */}
          <div className="prices-stats-card bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-200 dark:border-gray-700">
            <PriceStats
          ...
          {/* Price List/Grid - Enhanced Mobile Design */}
          {isLoading ? (
            <div className={`space-y-6 ${viewMode === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8' : ''}`}>
              {[...Array(6)].map((_, i) => (
                <div key={i} className={`border border-gray-200 dark:border-gray-700 rounded-xl p-5 sm:p-6 animate-pulse bg-white dark:bg-gray-800 shadow-sm ${viewMode === 'grid' ? 'h-56' : ''}`}>
            ...
            <div className="text-center py-12 px-4">
              <div className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-700 rounded-2xl p-8 mx-2 sm:mx-4 shadow-sm border border-gray-200 dark:border-gray-600">
                ...
                <h3 className="mt-4 text-lg font-medium text-gray-900 dark:text-white">
                  {t('prices.noPrices', 'Aucun prix trouvé')}
                </h3>
                <p className="mt-2 text-sm text-gray-500 dark:text-gray-400 max-w-md mx-auto">
                  {t('prices.noPricesDescription', 'Essayez d\'ajuster vos filtres ou soumettez le premier prix.')}
                </p>
                <div className="mt-6">
                  <Link to="/submit">
                    <Button className="prices-button bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white font-medium px-6 py-3 rounded-lg shadow-lg hover:shadow-xl transition-all duration-200">
                      <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                      </svg>
                      {t('prices.submitFirst', 'Soumettre le premier prix')}
                    </Button>
                  </Link>
                </div>
              </div>
            ...
            <div className={`prices-list-card ${viewMode === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8' : 'space-y-6 sm:space-y-8'}`}>
