# Script pour redémarrer le backend avec les corrections des endpoints d'administration
# Ce script arrête le backend existant et le redémarre avec les nouvelles corrections

Write-Host "=== REDÉMARRAGE DU BACKEND AVEC CORRECTIONS ===" -ForegroundColor Green
Write-Host ""

# 1. Arrêter le backend existant
Write-Host "1. Arrêt du backend existant..." -ForegroundColor Yellow
try {
    $javaProcesses = Get-Process -Name "java" -ErrorAction SilentlyContinue | Where-Object { $_.ProcessName -eq "java" }
    if ($javaProcesses) {
